...
class VPODE(nn.Module):
    ...
    def forward(self, t_k, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            # cond_energy_neg = self.ccf.get_cond_energy_single(z, self.y, 0)  #Energy weight in CCF class
            cond_energy_neg = self.ccf.get_cond_energy(z, self.y)  #Energy weight in CCF class
...
def main():
    ...
    for cls_list in pbar:
        ...
        for ii, cls_ in enumerate(cls_step_list):
            ...
            elif args.output_dir.split('/')[-1] == 'yelp':
                try:
                    pbar.set_description('Target: '+ cls_+'  Weight: '+str(weight_dict[cls_][0]))
                except:
                    pbar.set_description('Target: ' + cls_ + '  Weight: ' + '3.0')
            ...
            if weight_dict != None:
                ...
                # args.weight_energy = weight_dict[cls]
        ...
        condSampling = ConditionalSampling(sample_q, args.per_gpu_eval_batch_size, latent_dim, n_classes, classifier,
            ...
            # first_flag = False
        # else:
        #     condSampling.ccf = classifier
        # print('num of cls is ' + str(args.n_classes))
        # if args.n_classes>1:
        #     condSampling.get_samples()
        # else:
        #     condSampling.get_samples_length() #
        condSampling.get_samples_multiple(args.att_val_list.split(';'), mode=[1, 0, 0])
