...
from modules import VAE, DenseEmbedder, CCF, Similarity
from modules import ConditionalSampling
from modules import ConditionalTransferContent as ConditionalTransfer
...
class VPODE(nn.Module):
    def __init__(self, ccf,content_cls, y, beta_min=0.1, beta_max=20, T=1.0, save_path=None, plot=None, every_n_plot=5,
    ...
    def forward(self, t_k, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.reg_z > 0:
                ...
                z1 = self.content_cls(self.z_anchor)
                z2 = self.content_cls(z)
                cos_sim = self.sim(z1.unsqueeze(1), z2.unsqueeze(0))
                labels = torch.arange(cos_sim.size(0)).long().to(self.device)
                loss = torch.nn.CrossEntropyLoss()(cos_sim, labels)
                cond_energy_neg -= self.reg_z * loss
...
def sample_q_ode(ccf, content_cls, y, device=torch.device('cuda'), save_path=None, plot=None, every_n_plot=5, **kwargs):
    ...
    # y: batch
    # ODE function
    vpode = VPODE(ccf,content_cls, y, plot=plot, every_n_plot=every_n_plot, args=kwargs['args'], kwargs=kwargs)
...
#####################
    ...
    for cls_ in cls_step_list:
        classifier_ = DenseEmbedder(args.latent_size, up_dim=4, depth=4, num_classes=STEP_CLASSES[cls_])
    ...
    content_cls = DenseEmbedder(args.latent_size , up_dim=args.latent_size*2,depth=2, num_classes=args.latent_size*4)
    cls_dir = os.path.join(args.cls_dir + '0', 'training_cls.bin')
    cls_checkpoint = torch.load(cls_dir)
    content_cls.load_state_dict(cls_checkpoint['model_state_dict'], strict=False)
    content_cls.to(args.device)
    ...
    condSampling = ConditionalTransfer(sample_q, args.per_gpu_eval_batch_size, latent_dim, n_classes, classifier,
                                       ...
                                       every_n_plot=5, model_kwargs=model_kwargs, test_data_batch=eval_dataloader, content_cls=content_cls)
