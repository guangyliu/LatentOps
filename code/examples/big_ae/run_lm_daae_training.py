...
from modules import DAAE
from run_lm_vae_training import save_checkpoint, calc_rec_lgy,evaluate
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name):
    ...
    if args.local_rank in [-1, 0]:
        # tb_writer = SummaryWriter('./runs/'+args.output_dir.split('/')[-1])
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model_vae.named_parameters() if (not any(nd in n for nd in no_decay) and 'D' not in n)], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in model_vae.named_parameters() if any(nd in n for nd in no_decay) and 'D' not in n], 'weight_decay': 0.0}
    ]
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    scheduler = WarmupLinearSchedule(optimizer, warmup_steps=args.warmup_steps, t_total=t_total)
    optimizer_grouped_D = [{'params': [p for n, p in model_vae.D.named_parameters()],'weight_decay': 0.0 }]
    optimizer_D = AdamW(optimizer_grouped_D, lr=args.learning_rate, eps=args.adam_epsilon)
    scheduler_D = WarmupLinearSchedule(optimizer_D, warmup_steps=args.warmup_steps, t_total=t_total)
...
#                 loss_d = loss / args.gradient_accumulation_steps
            if args.fp16:
