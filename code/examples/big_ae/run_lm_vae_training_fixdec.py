...
from pytorch_transformers import (WEIGHTS_NAME, AdamW, WarmupLinearSchedule,
                                  BertConfig, BertForLatentConnector, BertTokenizer,BertForLatentConnectorNew,
                                  GPT2Config, GPT2ForLatentConnector, GPT2Tokenizer,GPT2ForLatentConnectorNew,
                                  OpenAIGPTConfig, OpenAIGPTLMHeadModel, OpenAIGPTTokenizer,
                                  RobertaConfig, RobertaForMaskedLM, RobertaTokenizer, RobertaForLatentConnector)
...
from modules import VAE, sample_sequence_conditional
...
def save_checkpoint(model_vae, optimizer, global_step, args):
    ...
    save_last = 1
    output_encoder_dir = os.path.join(args.output_dir, 'checkpoint-encoder-{}'.format(save_last))
    output_decoder_dir = os.path.join(args.output_dir, 'checkpoint-decoder-{}'.format(save_last))
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name, checkpoint=None):
    ...
    if args.fix_model == 0:  # no fix o
        ...
        optimizer_grouped_parameters = [
            {'params': [p for n, p in model_vae.named_parameters() if
                        ('linear' in n or 'decoder.transformer.h.0' in n or 'pooler' in n)],
             'weight_decay': 0.0}
        ]
        parameter_name.extend([n for n, p in model_vae.named_parameters() if
                               ('linear' in n or 'decoder.transformer.h.0' in n or 'pooler' in n)])
        ...
        parameter_name.extend([n for n, p in model_vae.named_parameters() if
                               ('encoder' in n or 'decoder.transformer.h.0' in n or 'linear' in n)])
    elif args.fix_model == 8:
        print('\ntrain BERT,  train wte wpe in GPT2\n')
        optimizer_grouped_parameters = [
            {'params': [p for n, p in model_vae.named_parameters() if
    ...
    scheduler = WarmupLinearSchedule(optimizer, warmup_steps=args.warmup_steps, t_total=t_total+500)
    print(optimizer)
    if args.fp16:
        try:
    ...
    with torch.no_grad():
        ...
        results.update(evaluate(args, model_vae, encoder_tokenizer, decoder_tokenizer, table_name))
    ...
    for epoch in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.disable_bar)
        for step, batch in enumerate(epoch_iterator):
            ...
            if train_step % 10 == 0:
                ...
                tb_writer.add_scalar('lr_train', scheduler.get_lr()[0], train_step)
...
def main():
    ...
    if args.fix_model == 3 or args.fix_model == 4:
        MODEL_CLASSES['gpt2'][1] = GPT2ForLatentConnectorNew
        MODEL_CLASSES['bert'][1] = BertForLatentConnectorNew
    elif args.fix_model == 5:
        # gpt2 unchanged
        MODEL_CLASSES['bert'][1] = BertForLatentConnectorNew
    elif args.fix_model == 6 or args.fix_model == 8:
        MODEL_CLASSES['gpt2'][1] = GPT2ForLatentConnectorNew
    ...
    if args.do_eval_rec and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            result = evaluate_rec(args, model_vae, tokenizer_encoder, tokenizer_decoder, table_name, prefix=global_step,
                                  subset='test')
