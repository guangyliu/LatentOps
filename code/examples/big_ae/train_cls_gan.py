...
from conditional_generation import latent2text
import sacrebleu
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name, classifier, gan):
    ...
    # scheduler_G = get_polynomial_decay_schedule_with_warmup(optimizer_G, args.warmup_steps, num_training_steps=t_total// args.n_cyc,
    #                                                       lr_end=1e-7, power=3.0)
    ...
    if args.fp16:
        ...
        if 'gan' in args.train_cls_gan:
            gan.latent_discriminator, optimizer = amp.initialize(gan.latent_discriminator, optimizer, opt_level=args.fp16_opt_level)
            gan.latent_generator, optimizer_G = amp.initialize(gan.latent_generator, optimizer_G, opt_level=args.fp16_opt_level)
        if 'cls' in args.train_cls_gan and 'gan' not in args.train_cls_gan:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        if 'cls' in args.train_cls_gan:
                            ...
                            # latent_z = gan.generate_z(20, eval=True)
                            # out = latent2text(model_vae,decoder_tokenizer,latent_z,True)
                            # tmp = classifier(latent_z).max(-1)[1]
                            # import pdb
                            # pdb.set_trace()
...
def distinct(lines):
    ...
    dist1 = round(len(set(grams_list1))/len(grams_list1),4)* 100
    dist2 = round(len(set(grams_list2))/len(grams_list2),4)* 100
    return (dist1,dist2)
...
def calc_ppl_lgy(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1, gan=None, model=None, tokenizer=None,
    ...
    #     sbleu.append(sacrebleu.corpus_bleu(list_of_references[i], refs).score* 100)
    # score = np.mean(sbleu)
...
def evaluate_acc(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1, classifier=None, gan=None):
    ...
    neg_correct = np.mean(neg_corrects)*2
    pos_correct = np.mean(pos_corrects)*2

    gan_acc_diff = np.mean(acc_diff_list)
    return {'acc': correct, 'gan_acc_diff': gan_acc_diff, 'loss': round(loss.mean().item(), 3),'neg_acc':neg_correct,'pos_acc':pos_correct}
