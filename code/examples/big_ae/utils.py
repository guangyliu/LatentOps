...
class BucketSampler(Sampler):
    def __init__(self, lens, bucket_size, batch_size, droplast=False, shuffle=True, sample=False):
        self._lens = lens
    ...
    def __iter__(self):
        ...
        if self.sample:
            batches = [ids[i * self._batch_size:(i + 1) * self._batch_size] for i in
                       range(int(len(self._lens) / self._batch_size))]
        return iter(batches)
...
class FeatureDataset(Dataset):
    ...
    def collate(features):
        input_ids_bert = pad_sequence([torch.tensor(f.input_ids_bert, dtype=torch.long) for f in features],
                                      batch_first=True, padding_value=0)
        input_ids_gpt = pad_sequence([torch.tensor(f.input_ids_gpt, dtype=torch.long) for f in features],
                                     batch_first=True, padding_value=0)
        lm_labels = pad_sequence([torch.tensor(f.input_ids_gpt, dtype=torch.long) for f in features], batch_first=True,
                                 padding_value=-1)
        return (input_ids_bert, input_ids_gpt, lm_labels)


class BucketingDataLoader(object):
    def __init__(self, file_path, batch_size, max_seq_length, tokenizer, args, bucket=100, shuffle=True, sample=False):
    ...
    def __iter__(self):
        sampler = BucketSampler(self.example_lengths, self.bucket_size, self.batch_size, droplast=True,
                                shuffle=self.shuffle, sample=self.sample)
...
class MultipleFiles_DataLoader(object):
    def __init__(self, file_path, batch_size, max_seq_length, tokenizer, args, bucket=100, shuffle=True,
        ...
        self.file_idx = 0
        self.cached_features_file = os.path.join(self.file_path,
                                                 args.dataset.lower() + f'.segmented.nltk.split.seq64.{self.file_idx}.json')
        self.dataset = PreparedTokenDataset(tokenizer, self.args, self.cached_features_file,
    ...
    def __iter__(self):
        ...
        loader = DataLoader(self.dataset, batch_sampler=sampler, num_workers=0,
                            collate_fn=PreparedTokenDataset.collate if self.use_tensor else PreparedTokenDataset.get_examples)
        yield from loader

        # update file name for next file
        self.file_idx += 1
        self.cached_features_file = os.path.join(self.file_path,
                                                 self.args.dataset.lower() + f'.segmented.nltk.split.seq64.{self.file_idx}.json')
        self.dataset = PreparedTokenDataset(self.tokenizer, self.args, self.cached_features_file,
                                            block_size=self.args.block_size)
        self.num_examples = len(self.dataset.examples)
...
class BucketingMultipleFiles_DataLoader(object):
    ...
    def __iter__(self):
        ...
        loader = DataLoader(self.dataset, sampler=sampler, batch_size=self.batch_size, pin_memory=True, num_workers=0,
                            collate_fn=PreparedTokenDataset.collate)
        yield from loader
...
class PreparedTokenDataset(Dataset):
    ...
    def get_examples(examples):
        token_lengths = torch.tensor([[f['bert_token_length'], f['gpt2_token_length']] for f in examples],
                                     dtype=torch.long)
...
class TokenDataset(Dataset):
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        cached_features_file = os.path.join(directory,
                                            'cached_lm_' + args.decoder_model_type + '_' + args.encoder_model_type + f'_{block_size}_{filename[:-4]}.json')
        ...
        self.cls = False
        self.content = False
        if 'Yelp' in args.dataset or 'cls' in args.dataset:
            label_on = True
        else:
            label_on = False
        if 'cls' in args.dataset:
            self.cls = True
        if 'content' in args.dataset:
            self.content = True
        if os.path.exists(cached_features_file):
            ...
            dropped, count = self._read_corpus_natural_split(fname=file_path, label=label_on, max_length=block_size,
                                                             block_size=block_size, args=args)
            ...
            for line in fin:
                ...
                if max_length:
                    if len(split_line_text.split()) > max_length:
                        ...
                        continue
                if self.new_version:
                    tokenized_text0 = self.tokenizers[0].encode(split_line_text, max_length=max_length, truncation=True)
                else:
                ...
                tokenized_text1 = self.tokenizers[1].encode(
                    ' '+split_line_text,max_length=max_length, truncation=True) # first token need a space
                #
                # self.tokenizers[1].convert_tokens_to_ids(self.tokenizers[1].tokenize(split_line_text))
                tokenized_text1 = [self.gpt2_bos_token] + tokenized_text1 + [self.gpt2_eos_token]
                tokenized_text1_length = len(tokenized_text1)
                # if tokenized_text0_length > 2 * max_length and tokenized_text1_length > 2 * max_length:
                #     dropped += 1
                #     continue
                example = {
...
class Dialog_TokenDataset(Dataset):
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            dropped, count = self._read_dialog_corpus_natural_split(fname=file_path, label=label_on,
                                                                    max_length=block_size, block_size=block_size,
                                                                    args=args)
    ...
    def collate(examples):
        # Convert to Tensors and build dataset
        input_ids_bert_ctx = pad_sequence([torch.tensor(f['bert_token_ctx'], dtype=torch.long) for f in examples],
                                          batch_first=True, padding_value=bert_pad_token)
        input_ids_bert = pad_sequence([torch.tensor(f['bert_token'], dtype=torch.long) for f in examples],
                                      batch_first=True, padding_value=bert_pad_token)
        input_ids_gpt = pad_sequence([torch.tensor(f['gpt2_token'], dtype=torch.long) for f in examples],
        ...
        token_lengths = torch.tensor(
            [[f['bert_token_ctx_length'], f['bert_token_length'], f['gpt2_token_length']] for f in examples],
            dtype=torch.long)
    ...
    def __init__(self, tokenizer, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            if text_split_mode == 'block':
                ...
                self._read_corpus_natural_split(fname=file_path, label=label_on, max_length=block_size,
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size):
        ...
        with open(fname) as fin:
            for line in fin:
                ...
                tokenized_text1 = tokenized_text1 + ([self.pad_token_id] * (
                        block_size - tokenized_text1_length - 2))  # Pad up to the sequence length.
    ...
    def _read_corpus_block_split(self, fname, block_size):
        ...
        while len(text) >= block_size:  # Truncate in block of block_size
            ...
            tokenized_text1 = tokenized_text1 + (
                    [pad_token_id] * (block_size - tokenized_text1_length - 2))  # Pad up to the sequence length.
            assert len(tokenized_text1) == block_size
    ...
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512, create_new=0):
        ...
        if not create_new and os.path.exists(cached_features_file):
            ...
            if text_split_mode == 'natural':
                if args.dataset == 'Yelp':
                    dropped = self._read_corpus_natural_split_yelp(fname=file_path, label=True, max_length=block_size,
                                                                   block_size=block_size)
                    logger.info("The number of dropped sentences is %d", dropped)
        ...
        with open(fname) as fin, open(label_fname) as lfin:
            for line, label_line in zip(fin, lfin):
                ...
                if block_size > tokenized_text0_length:
                    tokenized_text0 = tokenized_text0 + (
                            [pad_token] * (block_size - tokenized_text0_length))  # Pad up to the sequence length.
                else:
                    dropped += 1
                    continue
                ...
                if block_size > tokenized_text1_length:
                    tokenized_text1 = tokenized_text1 + ([self.pad_token] * (
                            block_size - tokenized_text1_length))  # Pad up to the sequence length.
                else:
                    dropped += 1
                    continue
                assert len(tokenized_text1) == block_size
                ...
                self.examples.append(
                    [tokenized_text0, tokenized_text0_length, tokenized_text1, tokenized_text1_length, lb])
...
class TextDataset_2Tokenizers(Dataset):
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            logger.info("Saving features into cached file 11 %s", cached_features_file)
            if args.use_philly:
                save_solid = False
                if not save_solid:
                    try:
    ...
    def _read_corpus_block_split(self, fname, block_size):
        ...
        while len(text) >= block_size:  # Truncate in block of block_size
            ...
            tokenized_text1 = tokenized_text1 + (
                    [pad_token] * (block_size - tokenized_text1_length - 2))  # Pad up to the sequence length.
            assert len(tokenized_text1) == block_size
...
def calc_iwnll(model_vae, eval_dataloader, args, ns=20):
    ...
    ppl_ = np.exp(-report_rec_loss / report_num_words)
    #     print(ppl_)
    return ppl, elbo, nll, kl


def calc_rec(model_vae, eval_dataloader, args, ns=1, rec_sample='default'):
    eval_loss = 0.0
    ############ Perplexity ############
    report_kl_loss = report_rec_loss = report_loss = 0
    report_num_words = report_num_sents = 0
    ...
    if rec_sample == 'default':
        ...
        for batch in tqdm(eval_dataloader, desc="Rec",disable=args.disable_bar):
            # 
            x0, x1, x_lengths = batch
            max_len_values, _ = x_lengths.max(0)
            x0 = x0[:, :max_len_values[0]]
            ...
            x0 = x0.to(args.device)
            x1 = x1.to(args.device)
            model_vae.rec_sample(x0, x1)
        return True
    elif rec_sample == 'sample':
        for batch in tqdm(eval_dataloader, desc="Sample",disable=args.disable_bar):
            # 
            x0, x1, x_lengths = batch
            ...
            x0 = x0.to(args.device)
            x1 = x1.to(args.device)
            model_vae.rec_sample(x0, x1, sample=True)
        return True
...
def calc_mi(model_vae, test_data_batch, args):
    ...
    neg_entropy = 0.
    for batch in tqdm(test_data_batch, desc="Evaluating MI, Stage 1", disable=args.disable_bar):
    ...
    num_examples = 0
    log_qz = 0.
    for i in tqdm(range(len(mu_batch_list)), desc="Evaluating MI, Stage 2",disable=args.disable_bar):
        ###############
        # get z_samples
        ###############
        mu_, logvar_ = mu_batch_list[i].cuda(), logvar_batch_list[i].cuda()

        # [z_batch, 1, nz]
        with torch.no_grad():
            z_samples = model_vae.reparameterize(mu_, logvar_, 1)
...
def calc_au(model_vae, eval_dataloader, args, delta=0.01):
    ...
    """
    cnt = 0
    for batch in tqdm(eval_dataloader, desc="Evaluating AU, Stage 1",disable=args.disable_bar):
    ...
    cnt = 0
    for batch in tqdm(eval_dataloader, desc="Evaluating AU, Stage 2",disable=args.disable_bar):
