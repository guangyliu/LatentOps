...
class TokenDataset(Dataset):
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        cached_features_file = os.path.join(directory, 'cached_lm_'+args.decoder_model_type+'_'+args.encoder_model_type+f'_{block_size}_{filename[:-4]}.json')
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size, args):
        ...
        with open(fname) as fin:
            for line in fin:
                ...
                #
                tokenized_text0 = self.tokenizers[0].encode(split_line_text)
                tokenized_text0_length = len(tokenized_text0)

                tokenized_text1 = self.tokenizers[1].encode(split_line_text) #self.tokenizers[1].convert_tokens_to_ids(self.tokenizers[1].tokenize(split_line_text))
                tokenized_text1 = [self.gpt2_bos_token] + tokenized_text1 + [self.gpt2_eos_token]
                tokenized_text1_length = len(tokenized_text1)
...
def calc_rec(model_vae, eval_dataloader, args, ns=1, rec_sample='default'):
    ...
    if rec_sample == 'default':
        ...
        for batch in tqdm(eval_dataloader, desc="Rec"):
            # pdb.set_trace()
            x0, x1, x_lengths = batch
            max_len_values, _ = x_lengths.max(0)
            x0 = x0[:, :max_len_values[0]]
        ...
        for batch in tqdm(eval_dataloader, desc="Sample"):
            # pdb.set_trace()
            x0, x1, x_lengths = batch
