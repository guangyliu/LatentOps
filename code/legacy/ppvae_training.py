...
from modules import VAE, DenseEmbedder, GAN, TinyVAE
...
def save_cls_checkpoint(classifier, optimizer, global_step, args, gan=None):
    ...
    output_cls_dir = os.path.join(args.output_dir, 'checkpoint-ppvae-{}'.format(save_last))
...
def LM_ppl():
    tokenizer = GPT2Tokenizer_LM.from_pretrained('gpt2')
    LM_model = GPT2LMHeadModel.from_pretrained('/home/lptang/text-autoencoders/classifier/gpt2_nounk_1/').cuda().eval()
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name, plugin_vae):
    """ Train the model """
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in plugin_vae.named_parameters() if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay},
        {'params': [p for n, p in plugin_vae.named_parameters() if any(nd in n for nd in no_decay)],
         'weight_decay': 0.0},
    ]
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            kl, mse_loss = plugin_vae(latent_z)
            loss = kl+mse_loss
            tb_writer.add_scalar('loss_plugin', loss.mean().item(), train_step)
            train_step += 1
            loss = loss.mean()
            loss.backward()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if args.fp16:
                    ...
                    torch.nn.utils.clip_grad_norm_(plugin_vae.parameters(), args.max_grad_norm)

                optimizer.step()
                # scheduler.step()  # Update learning rate schedule

                plugin_vae.zero_grad()
                # gan.latent_discriminator.zero_grad()

                epoch_iterator.set_description(
                    (
                        f'iter: {step + epoch * len(epoch_iterator)}; loss: {loss.item():.3f}; '
                        f'loss_kl: {kl.item():.3f}; loss_mse: {mse_loss.item():.3f}; '
                    )
                )
                global_step += 1
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate_acc(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1,
                                               plugin_vae=plugin_vae)
                        logger.info("MSE = %f", results['mse'])
                        if results['mse'] < best_cls_acc:
                            best_cls_acc = results['mse']
                            save_cls_checkpoint(plugin_vae, optimizer, global_step, args)
        ...
        if args.max_steps > 0 and global_step > args.max_steps:
            ...
            break
    results = evaluate_acc(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1, plugin_vae=plugin_vae)
...
def evaluate_acc(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1, plugin_vae=None, gan=None):
    eval_dataloader = build_dataload_and_cache_examples(args, [encoder_tokenizer, decoder_tokenizer], evaluate=True)
    corrects = []
    acc_diff_list = []
    for batch in tqdm(eval_dataloader, desc="Evaluating acc"):
        ...
        with torch.no_grad():
            ...
            latent_z = mean.squeeze(1)
            kl, mse_loss = plugin_vae(latent_z)
            corrects.append(mse_loss.float().cpu().numpy())
            ### GAN
...
def pp_samping(model_vae, plugin_vae,decoder_tokenizer, save_path):
    context_tokens = decoder_tokenizer.encode('<BOS>')
    f = open(os.path.join(save_path, 'sampling_pp.txt'), 'w')
    z = plugin_vae.sampling()
    valid_lines = []
    out = sample_sequence_conditional(
        model=model_vae.decoder,
        context=context_tokens,
    ...
    for i in range(z.size(0)):
        text_x1 = \
            decoder_tokenizer.decode(out[i, :].tolist(), clean_up_tokenization_spaces=False).split(
                ' <EOS>')[0]
        text_x1 = text_x1.split()[1:]
        text_x1 = ' '.join(text_x1)
        valid_lines.append('[BOS] ' + text_x1 + ' [EOS]')
...
def main():
    ...
    plugin_vae = TinyVAE(args)
    # classifier = DenseEmbedder(args.latent_size, 128, num_classes=args.n_classes)
    # gan = GAN(args)
    # pdb.set_trace()
    if args.use_random_weight:
        model_vae.apply(weights_init_rondom)
        plugin_vae.apply(weights_init_rondom)
    ...
    if args.do_train:
        ...
        global_step, tr_loss, optimizer = train(args, train_dataloader, model_vae, tokenizer_encoder, tokenizer_decoder,
                                                table_name, plugin_vae=plugin_vae)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        pp_samping(model_vae, plugin_vae, tokenizer_decoder, os.path.join(args.output_dir, 'sample'))
