...
from modules import VAE, DenseEmbedder, GAN
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name, classifier, gan):
    ...
    # loss = torch.tensor(0)
    pos_weight = torch.ones([args.n_classes-1]).cuda() *3
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if 'cls' in args.train_cls_gan:
                logits = classifier(latent_z)[:, :-1]
                num_classes = logits.size(-1)
                if num_classes > 1:

                    loss = criterion(logits, latent_labels)
                else:
            ...
            if best_acc_cnt >= 2 and best_diff_cnt > 5:
                break
...
def evaluate_acc(model_vae, encoder_tokenizer, decoder_tokenizer, args, ns=1, classifier=None, gan=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating acc"):
        x0, x1, x_lengths = batch
        max_len_values, _ = x_lengths.max(0)
        latent_labels = x_lengths
        # x0 = x0[:, :max_len_values[0]]
        x0 = x0.to(args.device)
        latent_labels = latent_labels.to(args.device)[:, :-1]
        with torch.no_grad():
            pooled_hidden_fea = model_vae.encoder(x0, attention_mask=(x0 > 0).float())[1]
            ...
            import pdb
            pdb.set_trace()
            loss = torch.nn.BCEWithLogitsLoss()(logits, latent_labels)
            # num_classes = logits.size(-1)
            # if num_classes > 1:
            ...
            corrects.append(loss.float().cpu().numpy())
            ### GAN
