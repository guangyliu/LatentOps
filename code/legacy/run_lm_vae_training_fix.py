...
#         ]
    linear_list = ['decoder.transformer.linear.weight','decoder.transformer.linear_emb.weight','encoder.linear.weight']
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model_vae.named_parameters() if n in linear_list], 'weight_decay': args.weight_decay}
        ]
...
def sampling_lgy(model_vae, tokenizer_decoder, args, LM_model, LM_tokenizer, cnt=200):
    ...
    with torch.no_grad():
        ...
        for i in range(valid_step):
            ...
            loss = F.cross_entropy(outputs.logits[:,:-1].reshape(-1,50260).contiguous(),labels.reshape(-1),ignore_index=50257)
